...
func (s *Store) Replace(ctx context.Context, userID string, hashes []string) error {
	...
	for _, h := range hashes {
		if _, err := tx.Exec(ctx, `INSERT INTO twofa_recovery_codes (user_id, code_hash) VALUES ($1,$2)`, userID, h); err != nil {
...
func (s *Store) List(ctx context.Context, userID string) (ids, hashes []string, err error) {
	rows, err := s.pool.Query(ctx,
		`SELECT id, code_hash FROM twofa_recovery_codes WHERE user_id = $1 AND used_at IS NULL`, userID)
	if err != nil {
...
func (s *Store) MarkUsed(ctx context.Context, id string) error {
	if _, err := s.pool.Exec(ctx, `UPDATE twofa_recovery_codes SET used_at = now() WHERE id = $1`, id); err != nil {
...
func (s *Store) DeleteAll(ctx context.Context, userID string) error {
	if _, err := s.pool.Exec(ctx, `DELETE FROM twofa_recovery_codes WHERE user_id = $1`, userID); err != nil {

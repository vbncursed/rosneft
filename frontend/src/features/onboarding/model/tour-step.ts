...
export interface TourStep {
  ...
  title: string;
  body: string;
  // The overlays panel tab this step's target lives on. The tour switches to it
  // before looking the target up, because an inactive tab is not in the DOM.
  // Spelled as the literal union rather than imported from the panel: a feature
  // may not reach up into a widget, and these are the same two strings.
  tab?: "view" | "placements";

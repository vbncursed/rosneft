...
### Task 5: Барьер ожидания восстановления (добавлена по итогам Task 4)

Живая проверка опровергла оценку «гонка в единицы миллисекунд»: чтение связки
ключей открывает диалог macOS и ждёт человека восемь секунд, за которые SPA
успевает получить 401 на `/api/auth/me` и стереть свой маркер. Барьер, стоявший
в спеке как отвергнутый вариант с оговоркой «если гонка окажется штатной»,
принят. Разбор — в спеке, пункт 4 решения.

**Files:**
- Modify: `desktop/src-tauri/Cargo.toml` (фичи tokio `sync`, `time`)
- Modify: `desktop/src-tauri/src/state.rs` (`AppState.restored`, `test_state`)
- Modify: `desktop/src-tauri/src/proxy.rs` (`RESTORE_WAIT`, `await_restore`, вызов в `forward`, три теста)
- Modify: `desktop/src-tauri/src/server.rs` (вызов в `handle_asset`)
- Modify: `desktop/src-tauri/src/main.rs` (`watch::channel`, `send(true)` на обеих ветках, устаревший комментарий)
- Modify: `CLAUDE.md`

- [x] Реализовано и закоммичено: `174dac1`. `make -C desktop check` — 80 тестов зелёные.

---

## Результат живой проверки (Task 4)

| проверка | результат |
| --- | --- |
| перезапуск открывает аккаунт | ✅ список территорий сразу, логина нет |
| `POST /api/auth/login` не вызывался | ✅ `cdat` записи keychain не изменился |
| `/api/auth/me` ответил 200 | ✅ `mdat` обновлён `store_session` |
| авторизованные GET прошли | ✅ три снапшота записаны |
| fallback порта не сработал | ✅ `is taken` в логе — 0 |
| второй экземпляр остановлен | ✅ вышел, не дойдя до `server::spawn`; порт у первого |
| запись keychain пережила неудачное восстановление | ✅ `mdat` не менялся, пока сессия не восстановилась |

Диалог связки ключей в дев-сборке остаётся при каждом запуске — это подпись
бинаря, не наш дефект; разбор и варианты в спеке, раздел «Что в объём не входит».

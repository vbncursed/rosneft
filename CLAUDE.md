...
- `GET /api/territories/{slug}/scene` — single-shot bundle (territory + LOD0 artifact + placements + placement groups + model options + saved measurements). Use this instead of four parallel calls. `measurements` is required and `[]` when the territory has none. `placementGroups` is required and `[]` when the territory has none.
...
- `POST /api/territories/{slug}/placements/batch` `{items: PlacementCreate[]}` (1–100) → 201 `Placement[]` in item order. One catalog transaction, so all or nothing; every item is pinned to the URL's territory. `placement:create`. An optional `Idempotency-Key` header (1–64 of `[A-Za-z0-9-]`, else 400) makes a retry safe: a key already used on that territory answers the rows it created (201, same order) and writes nothing, however old — before any panorama check, and with only the rows that remain if some were deleted since; two concurrent requests with one key write one batch and both answer it; a key naming an intact batch of another size is 409 `conflict`. Keys are per territory and kept in the catalog DB (`placements.idempotency_key`/`batch_index`, unique per territory), not in Redis. The SPA mints one per placing action (`use-placements-editor.ts`).
- `PUT /api/territories/{slug}/placements/hidden` `{ids, hidden}` and `PUT …/placements/group` `{ids, groupId: int64|null}` → `{updated: n}`: one catalog transaction over 1–1000 ids (a repeated id counts once), all or nothing. An id that is unknown or on another territory is 404, as is a group of another territory. `POST …/placement-groups` `{title}` → 201 `PlacementGroup`, `PATCH …/placement-groups/{id}` `{title}`, `DELETE …/placement-groups/{id}` → 204 (its placements return to no group). The title is trimmed, 1–120 characters, and unique on its territory compared case-insensitively (migration 00021; a clash is 400 on create and rename). All five are `placement:write`. `Placement` carries `hidden` (shared by every reader) and `groupId` (omitted when none); `PlacementCreate.groupId` lands a new placement in a group. oapi-codegen reads a missing `groupId` as `null`.
...
- `PATCH /api/auth/roles/{slug}` takes `permissionSlugs` beside `title`: present (even `[]`) replaces the grants in the rename's auth transaction, with the escalation check; absent leaves them. `PUT …/permissions` stays, and the SPA no longer calls it.
...
- `GET /api/console/summary` — Home's card numbers in one request: `users {total, frozen}`, `roles {roles, permissions}`, `content {territories, models}`, `access`, `audit24h`, `alerts`. A card the caller cannot open is absent (the gates copy `app/router/guard.ts` `SCREENS`, so change both together); a failed source is `null`. `no-store`.
...
- **A session whose account must enroll a second factor gets `403 {code: "twofa_enrollment_required"}` on everything** except `/api/auth/me`, `/api/auth/logout`, `/api/auth/2fa/setup`, `/api/auth/2fa/enable` and `/api/auth/2fa/recovery/regenerate` (`authhttp/enrollment.go`). The SPA's gate for it is `/two-factor-required`; see `frontend/CLAUDE.md`.
...
- **A 5xx body never echoes internal error text.** The generic one is `{code: "internal", message: "internal error"}` (`internalResp` in `httpapi`, `fail` in `authhttp`, `apperr.WriteStatus` for any 5xx), and what failed goes onto that request's slog-chi log line as `error` — one line, with the request id. A few routes answer their own fixed strings instead (`spec.go`, `audit_csv.go`, `watch_job_events.go`, the blob gate's 503, the metrics proxy's 502). A 4xx keeps the refusal's own words, and catalog and auth send them starting at the sentinel (`invalid input: panorama 5 is not on territory …`, `apperr.ToStatusAtSentinel`), never with the `service.X:` prefixes of the layers they passed through.

...
/** An SVG polyline through the series. Empty for an empty series. */
export function toLinePath(
  ...
  if (values.length === 1) {
    const y = scaleY(values[0], max, geo).toFixed(1);
...
/** The tallest value across every series, which they then share. */
export function sharedMax(series: { values: number[] }[]): number {

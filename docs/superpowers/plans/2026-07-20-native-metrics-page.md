...
- [x] **Step 2: Подключить правила в конфиге Prometheus**
...
- [x] **Step 3: Проверить синтаксис правил через promtool**
...
- [x] **Step 4: Смонтировать файл правил в compose**
...
- [x] **Step 5: Удалить сервис grafana из compose**
...
- [x] **Step 6: Переключить переменную окружения фронтенда**
...
- [x] **Step 7: Проверить, что упоминаний Grafana в конфигах не осталось**
...
- [x] **Step 8: Удалить каталог ops/grafana**
...
- [x] **Step 9: Проверить, что compose-файл валиден**
...
- [x] **Step 1: Написать падающий тест на series.ts**
...
- [x] **Step 3: Реализовать series.ts**
...
- [x] **Step 4: Убедиться, что тесты series проходят**

Run: `cd frontend && yarn test`
Expected: PASS для четырёх тестов из `series.test.ts`.

- [x] **Step 5: Написать падающий тест на panel.ts**
...
- [x] **Step 8: Убедиться, что все тесты проходят**
...
- [x] **Step 1: Создать реестр панелей**
...
- [x] **Step 2: Написать падающий тест на маппер**
...
- [x] **Step 2: Написать хук поллинга**
...
- [x] **Step 1: Написать оболочку панели**
...
- [x] **Step 3: Написать переключатель диапазона**
...
- [x] **Step 4: Написать карточку алертов**
...
- [x] **Step 5: Проверить типы и линт**
...
- [x] **Step 1: Написать композицию дашборда**
...
- [x] **Step 3: Удалить прокси Grafana**
...
- [x] **Step 4: Убедиться, что в коде не осталось следов Grafana**
...
- [x] **Step 5: Прогнать тесты, типы, линт и сборку**
...
- [x] **Step 1: Поднять стек с новым Prometheus**
...
- [x] **Step 2: Убедиться, что Prometheus загрузил правила**
...
- [x] **Step 3: Запустить фронтенд локально**
...
- [x] **Step 4: Проверить гейт роута без сессии**
...
### Task 9: Перевести интерфейс метрик на английский

Дефект исходного плана: он не зафиксировал язык интерфейса, и реализация внесла 34 русские строки
в кодовую базу, где весь остальной UI английский (`"No users."`, `"Could not add passkey"`,
`aria-label="Close panel"`). Заодно вычищаются два русских места вне `metrics/`, появившиеся раньше.

**Files:**
- Modify: `frontend/src/metrics/domain/panels.ts` (20 заголовков панелей + 2 секции)
- Modify: `frontend/src/metrics/presentation/components/metrics-dashboard.tsx`
- Modify: `frontend/src/metrics/presentation/components/range-picker.tsx`
- Modify: `frontend/src/metrics/presentation/components/alerts-card.tsx`
- Modify: `frontend/src/metrics/presentation/components/panel-card.tsx`
...
|---|---|
| `stat-up` | `Services up` |
| `stat-rps` | `Requests/sec` |
| `stat-errors` | `Error rate` |
| `stat-p99` | `p99 latency` |
| `stat-queue` | `Conversion queue` |
| `red-rate` | `gRPC requests by service` |
| `red-errors` | `gRPC errors by service` |
| `red-latency` | `gRPC p99 by service` |
| `red-http` | `HTTP requests` |
| `domain-conversions` | `Conversions by status` |
| `domain-conversion-p95` | `Conversion duration p95` |
| `domain-queue` | `Queue depth` |
| `domain-upload` | `Upload throughput` |
| `domain-auth` | `Logins by status` |
| `domain-twofa` | `2FA checks by status` |
| `runtime-memory` | `Resident memory` |
| `runtime-goroutines` | `Goroutines` |
| `runtime-gc` | `GC pause (max)` |
| `runtime-fds` | `Open file descriptors` |
| `alerts` | `Alerts` |
...
`range-picker.tsx`: `LABELS` → `{ "1h": "1h", "6h": "6h", "24h": "24h", "7d": "7d" }`,
`aria-label="Период"` → `aria-label="Time range"`.

`alerts-card.tsx`: заголовок `Алерты` → `Alerts`; ошибка
`Не удалось получить состояние алертов` → `Could not load alert state`; пустое состояние
`Всё спокойно — активных алертов нет.` → `All clear. No active alerts.`; бейдж
`активен` → `active`.
...
Run:
```bash
cd frontend && grep -rn "[А-Яа-яЁё]" src --include=*.tsx | grep -vE "^\S+: *//"
```
Expected: попадания только в комментариях. Ни одного JSX-текста, `aria-label`,
`placeholder` или строки в `title`.

Комментарии остаются русскими сознательно: в соседних инфраструктурных файлах
...
```bash
git add frontend/src
git commit -m "fix(metrics): render the metrics UI in English like the rest of the app"
```

---

### Task 10: Скользящая подложка в переключателе периода

Сейчас активный сегмент красит сам себя (`bg-white/10`), поэтому при клике выделение
перескакивает мгновенно. Нужна одна подложка на всю плашку, переезжающая между сегментами.
...
контрол. Своего изобретать не нужно.

Ключевые моменты: подложка рендерится **только** у активного сегмента, но `layoutId` у неё
один на все сегменты — поэтому Motion не гасит и зажигает её заново, а перемещает.
С кнопки снимается `bg-white/10` (цвет теперь даёт подложка), кнопка получает `relative`,
подпись оборачивается в `relative z-10`, иначе её перекроет абсолютная подложка.

Replace `frontend/src/metrics/presentation/components/range-picker.tsx`:
        ...
        return (
          ...
          >
            {/* Подложка одна на всю плашку: общий layoutId заставляет Motion
                переносить её между сегментами, а не перерисовывать на месте. */}
            {active && (
              <motion.span
...
- [ ] **Step 3: Пересобрать фронтенд и посмотреть глазами**

Run: `docker compose -p andrey up -d --build frontend`

Проверить на `/admin/metrics`: клик с `1h` на `24h` — подложка едет через промежуточные
...
git commit -m "feat(metrics): slide the range indicator between segments"
```

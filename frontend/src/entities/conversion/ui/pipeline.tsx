...
import { STAGE_DOT, type StageState } from "../model/status";
...
export function Pipeline({ steps, label = "Conversion pipeline", className }: PipelineProps) {
  return (
    ...
    <ol role="list" aria-label={label} className={cx("m-0 flex list-none flex-col gap-[9px] p-0", className)}>
      {steps.map((step) => {
        ...
        return (
          ...
          >
            <span aria-hidden="true" className={cx("absolute inset-y-0 left-0 w-[3px]", STAGE_DOT[step.state])} />
            <span
              aria-hidden="true"
              className={cx(
                "mt-[5px] size-2 shrink-0 rounded-full",
                STAGE_DOT[step.state],

...
| T5 | Closes on pointer leave, blur, **Esc** and on press of the trigger — pointer or keyboard (Enter/Space on it). Esc on a **focus-opened** tooltip is consumed (it must not also close the modal underneath); on a **hover-opened** one it closes the tooltip and carries on to the page (revised after PR #50 review: a hover tooltip over the Measure tile ate the Esc that leaves measure mode) | Standard; a pressed control has done its job, and a tooltip the mouse happened to wake must not stand between the page and its Esc |

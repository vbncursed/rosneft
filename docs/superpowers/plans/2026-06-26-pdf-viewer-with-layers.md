...
**Files:**
- Modify: `frontend/package.json` (add dep + `predev`/`prebuild` scripts)
- Create: `frontend/scripts/copy-pdfjs.mjs`
- Modify: `frontend/.gitignore` (ignore generated assets)

**Interfaces:**
- Produces: a populated `frontend/public/pdfjs/` tree containing `web/viewer.html`, `web/viewer.mjs`, `build/pdf.mjs`, `build/pdf.worker.mjs`, served at `/pdfjs/...`.

- [ ] **Step 1: Add `pdfjs-dist` as a dependency**

Run: `cd frontend && yarn add pdfjs-dist`
Expected: `package.json` `dependencies` gains `"pdfjs-dist": "^<version>"`; `yarn.lock` updated.

- [ ] **Step 2: Confirm the installed package's directory layout**

Run: `ls node_modules/pdfjs-dist`
Expected: includes `build/`, `web/`, and (usually) `cmaps/`, `standard_fonts/`. Confirm `web/viewer.html` and `build/pdf.worker.mjs` exist:
Run: `ls node_modules/pdfjs-dist/web/viewer.html node_modules/pdfjs-dist/build/pdf.worker.mjs`
Expected: both paths print (no "No such file"). If `cmaps`/`standard_fonts` are absent in this version, that's fine — the copy script skips missing dirs.

- [ ] **Step 3: Write the copy script** `frontend/scripts/copy-pdfjs.mjs`:

```js
// Copies pdf.js's prebuilt viewer + runtime into public/pdfjs so the document
// viewer can load /pdfjs/web/viewer.html. Runs on predev/prebuild; the output
// is gitignored (regenerated at build time, including inside Docker).
...
// web + build are required; cmaps + standard_fonts cover CJK / non-embedded
// fonts and are copied when present.
const dirs = ["build", "web", "cmaps", "standard_fonts"];

await rm(dest, { recursive: true, force: true });
for (const d of dirs) {
  const from = resolve(src, d);
  if (!existsSync(from)) {
    console.warn(`copy-pdfjs: skipping missing ${d}`);
    continue;
  }
  await cp(from, resolve(dest, d), { recursive: true });
}
console.log(`copy-pdfjs: vendored pdf.js viewer to ${dest}`);
```

- [ ] **Step 4: Wire the script to `predev` and `prebuild`** in `frontend/package.json` (Yarn/npm run `pre<script>` automatically before `dev`/`build`). Add to the `"scripts"` block:

```json
    "predev": "node scripts/copy-pdfjs.mjs",
    "prebuild": "node scripts/copy-pdfjs.mjs",
```

- [ ] **Step 5: Gitignore the generated tree** — append to `frontend/.gitignore`:

```
# vendored pdf.js viewer (generated by scripts/copy-pdfjs.mjs at build time)
/public/pdfjs/
```

- [ ] **Step 6: Run the copy and verify the assets exist**

Run: `cd frontend && node scripts/copy-pdfjs.mjs && ls public/pdfjs/web/viewer.html public/pdfjs/build/pdf.worker.mjs`
Expected: both files print; console shows "vendored pdf.js viewer to …/public/pdfjs".

- [ ] **Step 7: Confirm the tree is gitignored (not staged)**

Run: `git status --short frontend/public/pdfjs | head`
Expected: no output (ignored). `git status --short frontend/package.json frontend/yarn.lock frontend/scripts/copy-pdfjs.mjs frontend/.gitignore` should show those as changed/new.

- [ ] **Step 8: Commit**

```bash
git add frontend/package.json frontend/yarn.lock frontend/scripts/copy-pdfjs.mjs frontend/.gitignore
git commit -m "build(frontend): vendor pdf.js prebuilt viewer into public/pdfjs"
```

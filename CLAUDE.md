...
- `POST /api/territories/{slug}/placements/batch` `{items: PlacementCreate[]}` (1–100) → 201 `Placement[]` in item order. One catalog transaction, so all or nothing; every item is pinned to the URL's territory. `placement:create`.
...
- `POST /api/territories/{slug}/placements/batch` — 1–100 placements in one catalog transaction, all or nothing, answered in `items` order. An optional `Idempotency-Key` header (1–64 of `[A-Za-z0-9-]`, else 400) makes a retry safe: a key already used on that territory answers the rows it created (201, same order) and writes nothing, however old; two concurrent requests with one key write one batch and both answer it; a key naming a batch of another size is 409 `conflict`. Keys are per territory and kept in the catalog DB (`placements.idempotency_key`/`batch_index`, unique per territory), not in Redis. The SPA mints one per placing action (`use-placements-editor.ts`).
...
- **A 500 body is always `{code: "internal", message: "internal error"}`.** What failed goes onto that request's slog-chi log line as `error` (one line, with the request id) — `internalResp` in `httpapi`, `fail` in `authhttp`, and `apperr.WriteStatus` for any 5xx. A 4xx keeps the refusal's own words, and the catalog sends them starting at the sentinel (`invalid input: panorama 5 is not on territory …`), never with the `service.X:` prefixes of the layers they passed through.
